...
public class SuppliersProduct {
    ...
    public String deletesup(@PathVariable("id") int id, ModelMap model) {
        try {
            orderdetailRepository.deleteOrderDetailByProductId(productRepository.findProductsBySupplierID(id));
            productRepository.deleteProductBySupplierId(id);
            categoryRepository.deleteCategoryBySupplierId(id);
